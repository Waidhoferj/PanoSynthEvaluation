...
def main():
    ...
    parser.add_argument('--scene-dir',
                        help='directory containing .glb files for habitat rendering')
    parser.add_argument('--data-dir', '-o',
                        help='directory for testing data')
    parser.add_argument('--num-locations',
                        default=5,
                        type=int,
                        help='number of locations')
    parser.add_argument('--num-snapshots',
                        default=5,
                        type=int,
    ...
    if not args.data_dir:
        # Use inquirer
        answers = prompt([
        {"type": "confirm", "name": "should_generate_scenes", "message": "Generate testing data?", "default": True}
        ])
...
def generate_path():
    ...
    questions = [
        {"name": "scene_path", "type": "input", "message": "Path to the scene folder", "validate" : lambda a: os.path.isdir(a) or f"'{a}' is not a valid directory"},
        {"name": "output_path","type": "input", "message": "Testing data output path", "default": "data", },
        {"name": "location_count","type": "input", "message": "Number of locations per scene", "default": "5", "validate": is_int },
        {"name": "snapshot_count","type": "input", "message": "Number of snapshots per location", "default": "5", "validate": is_int},
    ]

    a = prompt(questions)
    generate_scene_data(a["scene_path"], a["output_path"], int(a["location_count"]), int(a["snapshot_count"]))
...
def generate_scene_data(scene_path, output_path, location_count, snapshot_count):
    ...
    for scene in scene_paths:
        ...
        for loc_i, pano_i in enumerate(location_indices):
            ...
            imwrite(os.path.join(location_path,"actual_depth.png"), depth.astype("uint8"))
            imwrite(os.path.join(location_path,"scene.jpeg"), cylinder_pano["rgba"].astype("uint8")[..., :3])

            disparity_map, layers = generate_mpi(os.path.join(location_path, "scene.jpeg"))
            imwrite(os.path.join(location_path, "predicted_depth.png"), disparity_map)
            os.makedirs(os.path.join(location_path, "layers"))
            for i, layer in enumerate(layers):
                imageio.imsave(os.path.join(location_path, "layers", f"layer_{i}.png"), (layer.numpy()* 255).astype("uint8"))
            ...
            for i in range(snapshot_count):
                ...
                with open(os.path.join(location_path, "poses", f"pose_{i}.json"), "w") as f:
                    json.dump(pose, f)
                imageio.imsave(os.path.join(location_path, "snapshots", f"snapshot_{i}.png"), snapshot)
...
def render_mci_snapshots(data_path):
    ...
    for location in locations:
        ...
        predicted_depth = imread(os.path.join(
            location, 'predicted_depth.png')).astype('float32')
        ...
        for path in pose_paths:
            with open(path) as f:
                p = json.load(f)
                poses.append((p["eye"] @ transform.T, p["target"] @ transform.T, p["up"] @ transform.T))
