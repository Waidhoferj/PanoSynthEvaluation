...
class Converter:
    def __init__(self, output_height, output_width, bottom=-1, top=1,):
        self.output_height = output_height
        self.output_width = output_width

        # make arrays of equally spaced angle and height values
        theta_range = np.linspace(-np.pi, np.pi,
                                  num=output_width, endpoint=False)
        height_range = np.linspace(
            bottom, top, num=output_height, endpoint=True)
        ...
        self.lutx = tf.cast(self.lutx, 'float32')
        self.luty = tf.cast(self.luty, 'float32')
